...
impl<T: RecentManagerImpl> RecentManagerImplExt for T {
    fn parent_changed(&self, recent_manager: &Self::Type) {
        unsafe {
            ...
            let parent_class =
                data.as_ref().get_parent_class() as *mut gtk_sys::GtkRecentManagerClass;
...
unsafe extern "C" fn recent_manager_changed<T: RecentManagerImpl>(
    ptr: *mut gtk_sys::GtkRecentManager,
) {
